        ...
        <div class="kehu_list">
          ...
          <div class="people_list">
            <ul>
              ...
              >
                <div class="list_box">
                  <el-badge :is-dot="item.isshow" class="item">
          ...
          <div class="chat_list" ref="Box" @scroll="orderScroll">
            <template v-for="(item, index) in right_chat">
              ...
              <div
                ...
                v-if="
                  item.from == active_id &&
                  item.content !== '%get your phone%' &&
                  item.content !== '%put my card%' &&
                  item.content.indexOf('data:image') == -1
                "
                :key="index + 'left'"
              >
              ...
              <!-- 业务员-发送分析报告-left -->
              <div
                class="sender_fenxi newcom"
                v-if="
                  item.from == active_id && item.content == '%get your phone%'
                "
                :key="index + 'fenxi'"
              >
                <div class="head_image">
              ...
              <!-- 默认的发送的项目-顾客发送 -->
              <div
                class="left_project new"
                v-if="
                  item.from == customer_id && item.content.indexOf('{') !== -1
                "
              >
                <div class="head_image">
              ...
              <div class="bao_box">
                ...
                <div class="xieyi">
                  ...
                  <p>
                    我已阅读并同意
                    <em
                      ><nuxt-link to="/we/fuwu"
                        >《{{ logo_text }}用户协议》</nuxt-link
                      ></em
                    >
                  </p>
                </div>
...
export default {
  ...
  methods: {
    ...
    baoMing() {
      ...
      if (checked == true) {
        ...
        this.$message.error(`请勾选${this.logo_text}用户协议`);
      }
    },
    showBox() {
      this.visible = true;
      ...
      if (e.data) {
        ...
        if (res.action == 203) {
          ...
          if (res.data.length == 0) {
            ...
            let list = res.data;
            list.map((v) => {
              if (v.content == "%put my card%") {
                v.content = "[收到名片]";
              } else if (v.content == "%get your phone%") {
                v.content = "[获取分析报告]";
              } else if (v.content.indexOf("data:image") !== -1) {
                v.content = "[我发送了一张图片]";
              } else if (v.content.indexOf("{") !== -1) {
                v.content = `[客户浏览了${JSON.parse(v.content).name}]`;
              } else {
                v.content = v.content;
              }
            });
            ...
            var result = this.people.some((item) => {
              if (item.id == this.active_id) {
                return true;
              }
            });
            if (result == true) {
              console.log("people列表已存在");
              this.people.map((m) => {
                if (m.id == this.active_id) {
                  this.online_name = m.name;
                }
              });
              this.show_mingpian = true;
              sessionStorage.setItem("list_prople_has", 2);
            } else {
              //获取当前业务员信息
              console.log("调用");
              let data = {
            ...
            this.people.map((p, q) => {
              if (p.id == this.active_id) {
                this.yewu_head_img = p.img;
              }
            });
          }
        } else if (res.action == 301) {
          let actived = this.active_id;
          this.tootip_show = false;
          ...
          if (this.active_id == res.fromUserName) {
            ...
            console.log("不相等", 301);
            //  this.yewu_head_img = res.from.head;
            //  this.online_name = res.from.name;
          ...
          console.log(sessionStorage.getItem(this.active_id), "active_id");

          if (sessionStorage.getItem(this.active_id) == 1) {
            //在线
            console.log(yewu_id, yewu_id !== res.fromUserName + "", "yewu_id");
            this.active_id = this.active_id;
            this.people.map((n) => {
              if (n.id == this.active_id) {
                this.yewu_head_img = n.img;
                this.online_name = n.name;
              }
            });
            ...
            this.people.map((p, q) => {
              if (
                p.id == res.fromUserName &&
                res.fromUserName !== this.active_id
              ) {
            ...
            if (res.content == "%get your phone%") {
              ...
              let data = {
                controller: "Staff",
                action: "info",
                params: { uuid: res.fromUserName, host: this.host },
              };
              this.websocketsend(JSON.stringify(data));
              this.show_mingpian = true;
            } else {
          ...
          if (chuang) {
            for (var i = chuang.length - 1; i >= 0; i--) {
          ...
          if (res.data.length > 0) {
            this.people.map((p, q) => {
              if (p.id == this.active_id) {
                let msg = "";
                if (res.data[res.data.length - 1].content == "%put my card%") {
                  msg = "[收到名片]";
                } else if (
                  res.data[res.data.length - 1].content == "%get your phone%"
                ) {
                  msg = "[获取分析报告]";
                } else if (
                  res.data[res.data.length - 1].content.indexOf(
                    "data:image"
                  ) !== -1
                ) {
                  msg = "[我发送了一张图片]";
                } else if (
                  res.data[res.data.length - 1].content.indexOf("{") !== -1
                ) {
                  msg = `[客户浏览了${
                    JSON.parse(res.data[res.data.length - 1].content).name
                  }]`;
                } else {
                  msg = res.data[res.data.length - 1].content;
                }
                p.content = this.zhuanBiao(msg);
          ...
          // if(this.show_mingpian==false){
          //   console.log('执行')
          //           let  msg ="";
          //           if(res.data[res.data.length-1].content=="%put my card%"){
          //                msg="[收到名片]";
          //           }else if(res.data[res.data.length-1].content=="%get your phone%"){
          //               msg="[获取分析报告]";
          //           }else if(res.data[res.data.length-1].content.indexOf('data:image')!==-1){
          //               msg="[图片]";
          //           }else{
          //               msg =res.data[res.data.length-1].content;
          //           }
          ...
          //            if(m.id==this.active_id){
          //                m.content = msg;
          //                m.time = res.data[res.data.length-1].createtime;
          //            }
          //        })
          // }
        } else if (res.action == 304) {
          console.log(res.visiting);
          sessionStorage.setItem(this.active_id, res.visiting);
          this.has_online = true;
        } else if (res.action == 307) {
        } else if (res.action == 206) {
          //返回的业务员名片
          ...
          if (sessionStorage.getItem("list_prople_has") == "1") {
            this.people.map((m, n) => {
              if (m.id == res.staff.id) {
                console.log("people列表已存在");
              } else {
                ...
                //去重
                let obj = {};
                let peon = people.reduce((cur, next) => {
                  obj[next.id] ? "" : (obj[next.id] = true && cur.push(next));
                  return cur;
                }, []);
                this.people = peon;
                this.yewu_head_img = res.staff.img;
              }
            });
            let uuid = localStorage.getItem("uuid");
            this.getChatliao(this.active_id, uuid, 1);
          ...
          if (res.staff.length == 0) {
            ...
            if (this.active_id == res.staff.id) {
              ...
              // console.log(this.yewu_head_img);
            } else {
              console.log("不相等", "305");
            }
  ...
  mounted() {
    ...
    sessionStorage.setItem("list_prople_has", 1);
    this.customer_id = localStorage.getItem("uuid");
    let flag = true;

    let _this = this;
    setTimeout(function () {
      if (_this.$route.name !== "jingjipeople") {
        _this.initWebSocket();
      }
    }, 2000);
