<template>
  <div class="header">
    <div class="header_cen">
      <div class="logo_box">
          <img src="~/assets/logo.png" alt v-if="this.$store.state.style_mine==0"/>
          <img src="~/assets/logo_yi.png" alt v-if="this.$store.state.style_mine==1"/>
        ...
        <div class="city_list" v-show="city_show">
            <h2>选择城市</h2> 
            <ul >
               <li v-for="item in this._props.all" :key="item.id"   @click="selCity(item)"  :class="item.area_id==active_num?'active':''">{{item.city}}</li>
            </ul>
        </div>
