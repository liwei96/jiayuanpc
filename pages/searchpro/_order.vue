  ...
  <div class="search">
    ...
    <div class="search_page">
      ...
      <div class="tiaojian_box">
        ...
        <div class="danjia">
          ...
          <div class="danjia_option">
            <div class="tiaojian">
              <el-radio-group v-model="danjias_model" >
                <el-radio
                  @click.native.prevent="changeDanjias(city.id)"
        ...
        <div class="zongjia">
          ...
          <div class="zongjia_option">
            <div class="hastiao">
              <el-radio-group v-model="zongjias_model">
                <el-radio
                 @click.native.prevent="changeZongjias(city.id)"
        ...
        <div class="mianji">
          ...
          <div class="mianji_option">
            <div class="hastiao">
              <el-radio-group v-model="mianjis_model" >
                <el-radio
                  @click.native.prevent ="changeMianjis(city.id)"
        ...
        <div class="fenlei" v-show="get_more_tiao">
          ...
          <div class="fenlei_option">
            <el-radio-group v-model="fenleis_model" >
              <el-radio
                @click.native.prevent="changeFenleis(city.type)"
      ...
      <div class="project_box_box">
          ...
          <ul class="project_box_m">
            <template v-if="info.length>0">
                <li v-for="item in info" :key="item.id">
                  <nuxt-link :to="`/content/${item.id}`" class="bg_top">
                      <img :src="item.img" :alt="item.name" :title="item.name"/>
                      <span class="butie" v-if="item.allowance!==0">
                        补贴 
                        <em>￥{{item.allowance}}</em>
                      </span>
                  </nuxt-link>
                  <div class="content">
                    <h2>
                      ...
                      >
                      <span class="daishou" v-if="item.state=='待售'">
                        <nuxt-link :to="`/content/${item.id}`"
                          >{{ item.state }}
                        </nuxt-link>
                      </span>
                       <span class="zaishou" v-if="item.state=='在售'">
                        <nuxt-link :to="`/content/${item.id}`"
                          >{{ item.state }}
                        </nuxt-link>
                      </span>
                       <span class="shoukong" v-if="item.state=='售空'">
                        <nuxt-link :to="`/content/${item.id}`"
                          >{{ item.state }}
                        </nuxt-link>
                      </span>
                    ...
                    <p class="tese">
                      <nuxt-link :to="`/content/${item.id}`">
                        <span class="jingzhuang" v-if="item.decorate">{{ item.decorate }}</span>
                        <span class="zhu" v-if="item.type">{{ item.type }}</span>
                        <span
                          class="tedian"
        ...
        <div class="right_hot">
          <!-- 返乡置业 -->
          <div class="fanxiang" v-show="allowance_num!==0">
              <img src="~/assets/fan/home_left.jpg" alt="">
              <button @click="goback">去选房</button>
              <p @click='showRules'>活动介绍></p>
          </div>
          <div class="topbox">
              ...
              <nuxt-link :to="`/content/${item.id}`">
                ...
                <div class="project_box">
                  <h2>
                    <i>{{ item.name }}</i>
                    <em>{{ item.type }}</em>
                    <span class="daishou" v-if="item.state=='待售'">{{ item.state }}</span>
                    <span class="zaishou" v-if="item.state=='在售'">{{ item.state }}</span>
                    <span class="shoukong" v-if="item.state=='售空'">{{ item.state }}</span>
                  </h2>
                  <p class="price">
                    <em>{{ item.price }}</em
                    >元/m²
...
export default {
  ...
  async asyncData({ app, query, params, store }) {
    ...
    let [data, project] = await Promise.all([
      app.$axios
        .get(
          `/search/condition?city=${store.state.city_id}&token=${store.state.token}&uuid=${uuid}`
        )
        .then((res) => {
    ...
    return {
      ...
      allowance_num:project.common.allowance_num
    };
  },
  data() {
    return {
  ...
  methods: {
    ...
    pageChange(num) {
      ...
      for (var key in has_arr) {
        if (key == "page") {
          this.tostring(key, has_arr[key],1);
        }
      }
    },
    show_zhaoafng() {
       ...
       if(val===this.danjias_model){
            ...
            //如果点击的对象是area就将v-model的值清空 radio状态为空
             ...
             if (has_arr) {
                for (var key in has_arr) {
                  if (key == "dprice") {
                    this.tostring(key, has_arr[key],0);
                  }
                }
            ...
            this.danjias_model = val ;
            //否则就把点击的值赋值给area 即绑定的radio
               //单价单选
              var data_id = val;
    ...
    changeZongjias(val) {  //总价
      if(val===this.zongjias_model){
          this.zongjias_model =[];
           let has_arr = JSON.parse(sessionStorage.getItem("typearr"));
             if (has_arr) {
                for (var key in has_arr) {
    ...
    //del_type ==1 不删除 del_type ==0 删除
    tostring(name, val,del_type) {
...
/deep/ 
.zongjia_option{ //checkbox 总价设置宽度
   .el-radio-group{
        .el-radio{
          width: 105px;
          margin-right: 0px;
