  ...
  <div class="fang_zi">
    ...
    <div class="fangzi_page">
       ...
       <div class="dong_left">
            <el-timeline>
              <template v-for="(item,index) in data" >
                <el-timeline-item :timestamp="item.time" placement="top" v-if="index==0" :key="item.id">
                  <div class="dong_content">
                      <h4><nuxt-link :to="`/loudongpage/${item.id}`">{{item.title}} <span class="new">最新</span></nuxt-link></h4>
                      <p><nuxt-link :to="`/loudongpage/${item.id}`">{{item.content}}</nuxt-link>
                      </p>
                       <a href="" v-if="item.content.length>68"><nuxt-link :to="`/loudongpage/${item.id}`">查看详细>></nuxt-link></a>
                      <ul class="imglist">
                          <li><nuxt-link :to="`/loudongpage/${item.id}`"><img :src="item.img" alt=""></nuxt-link></li>
                      </ul>                
                  </div>
              </el-timeline-item>
              </template>
              ...
              <template v-for="(item,index) in data">
                <el-timeline-item :timestamp="item.time" placement="top" v-if="index==1" :key="item.id">
                  <div class="dong_content">
                      <h4><nuxt-link :to="`/loudongpage/${item.id}`">{{item.title}} <span class="jiatui">加推</span></nuxt-link></h4>
               ...
               <template v-for="(item,index) in data">
                  <el-timeline-item :timestamp="item.time" placement="top" v-if="index>1" :key="item.id">
                      <div class="dong_content">
                        <h4><nuxt-link :to="`/loudongpage/${item.id}`"> {{item.title}}</nuxt-link></h4>
                        <p><nuxt-link :to="`/loudongpage/${item.id}`">{{item.content}}</nuxt-link></p>
                        <a href="" v-if="item.content.length>68"><nuxt-link :to="`/loudongpage/${item.id}`">查看详细>></nuxt-link></a>
                        <ul class="imglist">
                          <li><nuxt-link :to="`/loudongpage/${item.id}`"><img :src="item.img" alt=""></nuxt-link></li>
      ...
      <div class="right_zixun">
        ...
        <div class="hot_news">
          <h3>买房百科 <span>
            <nuxt-link to="/baike?type=56&page=1">更多>
            </nuxt-link>
            </span></h3>
          <ul>
...
export default {
    ...
    head() {
    return {
      title:`${this.logo_text}新房-楼盘动态` ,
      meta: [
        {
          hid: "description",
          name: "description",
          content: `${this.logo_text}新房-楼盘动态`
        },
        { hid: "Keywords", name: "Keywords", content: `${this.logo_text}新房-楼盘动态` }
      ]
    };
  },
  watchQuery:["page"],
  components: {
    ...
    async asyncData({ app, query, params, store }) {
      ...
      let [data,article] = await Promise.all([
       ...
       //   ***楼盘动态部分***
        app.$axios.get(`/jy/dynamic/info/phone?city=${store.state.city_id}&page=${page}&uuid=${uuid}`).then((res) => {
          if (res.data.code == 200) {
            let data = res.data;
            return data;
          }
  ...
  methods: {
    tijiaoDong(num) {
      ...
      pageChange(num) {
            this.$router.push(`/fangzidong?page=${num}`);
        }
  },
};
</script>
