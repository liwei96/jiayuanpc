    ...
    <div class="fangzi_page">
      <div class="left_zixun">
        <div class="sou_rasult">
          <h2>
            共为您搜索到<span>{{ this.total }}</span
            >条关于“<span>{{ this.value }}</span
            >”的资讯
          </h2>
        </div>
        <!-- 中间文章 -->
          ...
          <li v-for="item in data" :key="item.id">
            <nuxt-link :to="`/article/${item.id}`">
              ...
              <div class="text_box">
                ...
                <p class="time">
                  {{ item.time }}<span>来源：{{ item.source }}</span>
                </p>
              </div>
            </nuxt-link>
          </li>
      ...
      <div class="right_zixun">
        ...
        <div class="hot_news">
          <h3>
            买房百科
            <span>
              <nuxt-link to="/baike?type=56&page=1"> 更多> </nuxt-link>
            </span>
          </h3>
          <ul>
        ...
        <div class="tui_lou">
          ...
          <button>
            <nuxt-link to="/searchpro">更多楼盘 </nuxt-link>
          </button>
        </div>
      </div>
    </div>
...
export default {
  ...
  async asyncData({ app, query, params, store }) {
    ...
    let [data, article] = await Promise.all([
      ...
      app.$axios
        .get(
          `/jy/article/search?city=${store.state.city_id}&token=${store.state.token}&uuid=${uuid}`
        )
        .then((res) => {
      ...
      // page 第几页
      app.$axios
        .get(`/api/article/e_search?name=${name}&page=${page}&uuid=${uuid}`)
        .then((res) => {
          if (res.data.code == 200) {
            let data = res.data;
            return data;
