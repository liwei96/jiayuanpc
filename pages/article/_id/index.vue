    ...
    <div class="artice">
      <div class="left_article">
        ...
        <p class="fuze">
          ...
          凡本站注明 “来自：XXX(非{{
            logo_text
          }}网)”的资讯稿件和图片作品，系本站转载自其它媒体，转载目的在于信息传递，并不代表本站赞同其观点和对其真实性负责。
          如有资讯稿件和图片作品的内容、版权以及其它问题的，请联系本站，电话：{{
            common.phone
          }}
        </p>
        <div class="zan">
          <span class="zan_btn" @click="getLike">
            <img
              ...
              alt=""
              v-if="article.my_like == 0"
            />
            <img
              src="~/assets/icon/haszan.png"
              alt=""
              v-if="article.my_like == 1"
            />
            点赞
          </span>
...
export default {
  ...
  async asyncData({ app, query, params, store }) {
    ...
    let [data] = await Promise.all([
      ...
      app.$axios
        .get(
          `/jy/article/detail?id=${id}&city=${store.state.city_id}&token=${store.state.token}&uuid=${uuid}`
        )
        .then((res) => {
