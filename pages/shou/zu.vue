      ...
      <ul>
          <template  v-if="list.length>0" >
            <li v-for="item in list" :key="item.id"  @click="goDetail(item.id)">
                    <nuxt-link :to="`/content/${item.id}`"><img :src="item.img" alt=""></nuxt-link>
                    <div class="rig">
                          <p class="title"><nuxt-link :to="`/content/${item.id}`">{{item.name}} <span>{{item.state}}</span></nuxt-link>
                          ...
                          <p class="gu">新房咨询师： <span @click.stop="bianTong(104,item.id)"><strong  class="iconfont iconzixun1"></strong>向他咨询</span> <i>参考总价：{{item.total}}万元/套起</i></p>
...
export default {
    ...
    head () {
      return {
        title: "我的足迹",
        meta: [
          { hid: 'description', name: 'description', content: '我的足迹' }
        ]
      }
    },
    async asyncData({app,store,error,query}){
      ...
      if(uuid && uuid!=="undefined" && uuid!=="null"){
        ...
        let {data} =await app.$axios.get('/jy/mine/foots',{
                    params:{
