<template>
  <div class="jian_box">
    ...
    <ul>
      <template v-if="list.length > 0&&total!=0">
        <li v-for="item in list" :key="item.id" @click="goDetail(item.id)">
          ...
          <div class="rig">
            <p class="title">
              <nuxt-link :to="`/content/${item.id}`"
                >{{ item.name }} <span>{{ item.state }}</span></nuxt-link
              >
              <!-- <i><strong class="iconfont iconlujing"></strong>收藏</i> -->
            </p>
            <p class="dizhi">{{ item.country }} | {{ item.address }}</p>
            <p class="hu">
              <!-- {{ JSON.stringify(item.house_types).replace(/\[|]/g, "") }} -->
              {{ item.type }} |
              <em>面积：{{ item.area }}m²</em>

              <span
                >均价<strong>{{ item.price }}</strong
                >元/m²</span
              >
            </p>
            <p class="gu">
              新房咨询师：
              <span @click.stop="bianTong(104, item.id)"
                ><strong class="iconfont iconzixun1"></strong>向他咨询</span
              >
              <i>参考总价：{{ item.total }}万元/套起</i>
            </p>
            <p class="te">
              <span>{{ item.decorate }}</span> <span>{{ item.type }}</span>
            </p>
          </div>
        </li>
      </template>
      <div class="no_project" v-else>您暂时没有浏览的足迹哦</div>
    </ul>
    <div class="page">
      <el-pagination
        background
...
export default {
  ...
  head() {
    return {
      title: "我的足迹",
      meta: [{ hid: "description", name: "description", content: "我的足迹" }],
    };
  },
  ...
  methods: {
    bianTong(num, project_id) {
      ...
      switch (num) {
        case 104:
          this.$parent.$parent.$parent.$parent.baoming_tel.tan_title =
            "向TA咨询"; //ok
    ...
    pageChange(num) {
      this.$router.push(`/shou/zu?page=${num}`);
    },
  },
  beforeMount() {
