<template>
  <div class="prohuxing">
    ...
    <my-mian-bao :router_data="routerData"></my-mian-bao>
    <div class="prohuxing_box">
        ...
        <div class="hu_left">
          <ul>
            <li v-for="item in rooms" :key="item.id">
              <div class="top_hu" >
                <nuxt-link :to="`/hudetail/${building.id}`">
                  <img
                    :src="item.small"
                    :alt="`${building.name}${item.title}`"
                    :title="`${building.name}${item.title}`"
                  />
                  <span>
                    <em class="el-icon-cloudy"></em>
                    <i>{{item.num}}关注</i>
                  </span>
                </nuxt-link>
              </div>
              <div class="bottom_hu">
                <p class="hu_name">
                   <nuxt-link :to="`/hudetail/${building.id}`">
                    ...
                    <em>
                      约
                      <i v-if="item.price>0 && item.price!==0">{{item.price}}</i>
                      <i v-else>待定</i>万/套
                    </em>
                   </nuxt-link>
                </p>
                <p class="hu_mian">
                   <nuxt-link :to="`/hudetail/${item.id}`">
                      <span>面积 {{item.area}}m²</span>
                      <em>{{item.state}}</em>
                   </nuxt-link>
                </p>
                <button @click="bianTong(109)">剩余房源查询</button>
              </div>
            </li>
          </ul>
          <div class="hu_jieshao">
            <h3>项目介绍</h3>
            <p>{{building.introduce}}</p>
          </div>
          <!-- 底部报名框 -->
          <div class="bottom_baoming">
              <div class="bao_box">
                  <p>{{this.text}}</p>
                  <div class="bao_input">
          ...
          <!-- <bottom-baoming :mytext="text"></bottom-baoming> -->
        </div>
        <div class="hu_right">
          <!-- 楼盘分析师 -->
          <div class="lou_fen">
            <div class="loufen_top">
              ...
              <p>
                <span>
                  <em class="iconfont iconanquan"></em>专业服务
                </span>
                <span>
                  <em class="iconfont iconanquan"></em>区域解读
                </span>
                <span>
                  <em class="iconfont iconanquan"></em>户型分析
                </span>
              </p>
            </div>
            ...
            <ul>
              <li v-for="(item,index) in common.staffs" :key="index">
                ...
                <div class="name">
                  <h5>{{item.staff.name}}</h5>
                  <p>
                    服务人数
                    <em>{{item.staff.ServeNum}}</em>人
                  </p>
                </div>
          ...
          <my-bao-ming :img_bg="img_bg_data" :text_tishi="text" ref="right_bao"></my-bao-ming>
...
//import bottom_baoming from "~/components/danpage/baomingkuang.vue";
...
export default {
  ...
  components: {
    ...
    "my-mian-bao": mianbao,
    "my-bao-ming": baoming,
   // "bottom-baoming": bottom_baoming,
    "my-footer": footer,
    'my-tu-yan':tuyanMa,
  },
  data() {
    return {
      text:"好楼盘户型是关键，咨询详细户型信息，安安心心买房",
      building:{},
      rooms:[],
      common:{},
      img_bg_data:{
          backgroundImage: "url(" + require("~/assets/hu_bg.jpg") + ")",
